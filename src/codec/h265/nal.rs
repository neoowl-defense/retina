...
impl Pps {
    pub fn from_bits<R: BitRead>(mut r: R) -> Result<Self, Error> {
        ...
        if pps_scaling_list_data_present_flag {
            // Skip scaling list data - we don't need it for recording
            // The existing ScalingListData parser can fail on some camera streams
            // We already have the two fields we need (tiles_enabled_flag, entropy_coding_sync_enabled_flag)
            // so we can return early if parsing fails
            if let Err(_e) = ScalingListData::from_bits(&mut r) {
                // Scaling list parsing failed - that's okay, we have what we need
                // Return early with the flags we already parsed
                return Ok(Self {
                    tiles_enabled_flag,
...
/// Skip over HRD parameters in H.265 bitstream
/// T.REC H.265 section E.2.2 `hrd_parameters`
/// This is a simplified implementation that skips the HRD parameters without parsing them fully.
/// HRD parameters are only needed for playback timing/buffering analysis, not for recording.
fn skip_hrd_parameters<R: BitRead>(r: &mut R) -> Result<(), Error> {
    // nal_hrd_parameters_present_flag
    let nal_hrd_present = r.read_bool("nal_hrd_parameters_present_flag")?;
    // vcl_hrd_parameters_present_flag
    let vcl_hrd_present = r.read_bool("vcl_hrd_parameters_present_flag")?;
    ...
    if nal_hrd_present || vcl_hrd_present {
        ...
        if sub_pic_hrd_params_present {
            let _: u8 = r.read(8, "tick_divisor_minus2")?;
            let _: u8 = r.read(5, "du_cpb_removal_delay_increment_length_minus1")?;
            let _ = r.read_bool("sub_pic_cpb_params_in_pic_timing_sei_flag")?;
            let _: u8 = r.read(5, "dpb_output_delay_du_length_minus1")?;
        }

        let _: u8 = r.read(4, "bit_rate_scale")?;
        let _: u8 = r.read(4, "cpb_size_scale")?;

        if sub_pic_hrd_params_present {
            let _: u8 = r.read(4, "cpb_size_du_scale")?;
        }

        let _: u8 = r.read(5, "initial_cpb_removal_delay_length_minus1")?;
        let _: u8 = r.read(5, "au_cpb_removal_delay_length_minus1")?;
        let _: u8 = r.read(5, "dpb_output_delay_length_minus1")?;

        // For simplicity, skip the sub_layer_hrd_parameters
        // In a full implementation, we would need to parse maxNumSubLayersMinus1 from SPS
        // and iterate through each sub-layer, but for recording purposes we can skip this
    }
...
impl VuiTimingInfo {
    pub fn from_bits<R: BitRead>(r: &mut R) -> Result<Self, Error> {
        ...
        if hrd_parameters_present_flag {
            // Skip HRD parameters - we don't need them for recording (only for playback timing)
            // This is a workaround for cameras that include HRD parameters in H.265 SPS
            // TODO: Implement full HRD parameter parsing per H.265 spec E.2.2
