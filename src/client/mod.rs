...
impl Session<Playing> {
    ...
    fn handle_response(
        mut self: Pin<&mut Self>,
        _msg_ctx: &crate::RtspMessageContext,
        response: rtsp_types::Response<Bytes>,
    ) -> Result<(), Error> {
        let inner = self.0.as_mut().project();
        // Extract cseq and method from either Waiting or Flushing state
        // (response can arrive while we're still flushing the request)
        let (expected_cseq, method) = match inner.keepalive_state {
            KeepaliveState::Waiting { cseq, method } => (Some(*cseq), Some(*method)),
            KeepaliveState::Flushing { cseq, method } => (Some(*cseq), Some(*method)),
            KeepaliveState::Idle => (None, None),
        };

        let response_cseq = parse::get_cseq(&response);

        // Check if this response matches our pending keepalive
        if let (Some(expected), Some(method)) = (expected_cseq, method) {
            if response_cseq == Some(expected) {
        ...
        // Response with wrong CSeq - camera is buggy but clearly alive
        // Treat any response as proof the camera is alive (the purpose of keepalives)
        // and reset state to prevent timeout errors
        if expected_cseq.is_some() {
            warn!(
                "Keepalive got response with wrong CSeq (expected {:?}, got {:?}, status {:?}) - camera alive, resetting",
                expected_cseq,
                response_cseq,
                response.status()
            );
            *inner.keepalive_state = KeepaliveState::Idle;
            ...
            warn!(
                "Ignoring unexpected RTSP response (no keepalive pending, CSeq {:?}, status {:?})",
                response_cseq,
                response.status()
            );
        }
